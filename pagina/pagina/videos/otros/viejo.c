...
//se leen los parametros del cliente para ver que pide
   ...
   if (cont==0){// este es el caso en el que se crea el comprimido
     ...
     // system("md5sum -c md5Checksum.md5sum");
      ...
      system("md5sum -c md5Checksum.md5sum");
      system("ffmpeg -i videoReporte.mjpeg -r 25 -pix_fmt yuv420p -strict -2 -acodec aac -b:a 128k -vcodec libx264 -crf 21 -rc-lookahead 250 videoReporte.mp4");
      /*system("cat mjpegtomp4.txt nombrevideo.txt>nombre.txt");
   ...
   if (pFile == NULL) perror ("Error opening file");
     ...
     if ( fgets (mystring , 136 , pFile) != NULL )
       puts (mystring);
     fclose (pFile);
